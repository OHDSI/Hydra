...
The first step is to fully define the study in [ATLAS](http://atlas-demo.ohdsi.org/). This can be an estimation study of a prediction study. Once completed, we need to obtain the JSON. We can obtain the JSON from the Export tab in the Utilities tab, as shown in Figure 1.
...
Select all the JSON text (for example using Ctrl-a on Windows), copy it, and paste it into a text file. Save the text file somewhere on your computer, for example in `c:/temp/specs.json`.
